...
class TestRunner {
  ...
  filterTests() {
    ...
    return this.testCases.filter((test) => {
      ...
      return (
        (description.includes(filter) || category.includes(filter)) &&
  ...
  runTest(testCase) {
    ...
    try {
      ...
      if (typeof this.solution.solveAlternative === 'function') {
        const { result: altResult, executionTime: altExecutionTime } =
          this.executeAlternativeTest(input);
  ...
  validateResult(result, expected, validator) {
    return validator ? validator(result, expected) : Helpers.compareValues(result, expected);
  }
